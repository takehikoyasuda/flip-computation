...
1. ~~**正規性判定**~~ **完了。R1 は判定不要だった。**
   ...
   * 「`(I^(m))^k = I^(km)`（divisorial algebra が degree 1 生成）で代用」→
   ...
   残る限界: 相対素点での A の S2 と Z の S2 の同一視は `A_f = A_(f)[f,f^{-1}]`
   を使うので **f の次数が1** を要る。`u_i` は常に次数1なのでアフィン基底と標準
   次数の射影基底では厳密だが、**重み付き射影基底では錐の判定にフォールバック**
   している（十分条件）。重み付きでも厳密にできるかは未検討。
   なお現時点で3つの判定が食い違う例は見つかっていない。
