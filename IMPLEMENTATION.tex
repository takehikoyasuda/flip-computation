...
\item
  ...
  \(I = \sO_X(-E)\);
\item
  for increasing \(m\), until one passes the test of Step 5 ---
  termination guaranteed because every sufficiently divisible \(m\)
  passes it;
\item
...
Three departures matter mathematically. The first is a correction; the
...
the canonical class the divisor package returns, and \emph{which}
representative that is has changed under the package's own maintenance.
Under the grading \(\ell = (2,2,1)\): with Macaulay2 1.24 and
\texttt{Divisor} the section was \(s = y_3^{11}\), \(I^{(1)}\) landed in
degree 30, and the computation did not finish in seventeen minutes; with
1.26 and \texttt{WeilDivisors} it is \(s = 1\) and degree 8. The direct
embedding gives degree 2 under both. So the size of the penalty is a
fact about a package version, but its existence is not --- and neither
is the reason for it, which is that Step 2 has no way to ask for a small
representative and the embedding does. The regression test asserts the
gap rather than either number.

One further choice is worth recording, though it is not a departure.
Step 4 stops at the first \(m\) that passes, and termination rests on
every sufficiently divisible \(m\) passing; \emph{which} \(m\) are tried
on the way is left open. One \(m\) that works is as good as another, so
what matters is reaching a small one, and the cost per iteration grows
steeply: on the threefold below, 0.02 s at \(m = 1\) against more than
600 s at \(m = 12\). The implementation takes every divisor of
\(\texttt{MaxSteps}!\) in increasing order. That sequence contains
\(1!, 2!, \dots, \texttt{MaxSteps}!\), so it is eventually sufficiently
divisible and the guarantee is intact, while filling the gaps between
the factorials costs almost nothing --- \(1,2,3,4,6,8\) together take
8.5 s, against more than 600 s for \(m = 12\) alone. Enumerating every
integer instead would have to run to \(\texttt{MaxSteps}!\) for the same
guarantee: 24 iterations where 8 suffice, and the extra ones are the
expensive, unlikely ones.

\section{Environment and
reproducibility}\label{environment-and-reproducibility}

Computed with Macaulay2 1.26.06 on macOS (Apple Silicon), and previously
with 1.24.05 and 1.24.11, using the bundled packages
\texttt{WeilDivisors}, \texttt{SymbolicPowers}, \texttt{Polyhedra},
\texttt{IntegralClosure}, \texttt{MinimalPrimes} and
\texttt{Elimination}. The first of these was called \texttt{Divisor}
through 1.24, which is the name the measurements above are quoted
...
The systems of equations quoted below are exactly what this version
returns. They are Gr\"obner basis output, so the order in which the
generators are listed --- though not the ideal they generate, and not
any statement made about it here --- may differ under another version of
Macaulay2.
...
The Hilbert basis of \(\sigma^\vee \cap M\) has five elements, so this
\(X\) is a threefold in \(\bbA^5\), and the input handed to the
algorithm is the three quadrics cutting it out:

\begin{verbatim}
y_4^2-y_2*y_5     y_3*y_4-y_1*y_5     y_2*y_3-y_1*y_4
\end{verbatim}

\noindent
equivalently, the \(2 \times 2\) minors of

\[\begin{pmatrix} y_1 & y_2 & y_4 \\ y_3 & y_4 & y_5 \end{pmatrix}.\]
...
\noindent
and returns \(Z \subset \PP^1 \times X\) cut out by

\begin{verbatim}
y_4^2-y_2*y_5     y_3*y_4-y_1*y_5     u_1*y_4-u_2*y_5
y_2*y_3-y_1*y_4   u_1*y_2-u_2*y_4     u_1*y_1-u_2*y_3
\end{verbatim}

\noindent
The three equations of \(X\) survive, and three more appear, each linear
in \(u\). Together they are once again the \(2 \times 2\) minors of a
single matrix --- the matrix of \(X\) with one column adjoined:

\[I_2\begin{pmatrix} y_1 & y_2 & y_4 \\ y_3 & y_4 & y_5 \end{pmatrix}
  \;\rightsquigarrow\;
  I_2\begin{pmatrix} u_2 & y_1 & y_2 & y_4 \\
                     u_1 & y_3 & y_4 & y_5 \end{pmatrix}.\]

On this example, then, the flip is the operation of adjoining the fibre
coordinates as a new column. The identity is not read off by eye: the
ideal returned by \texttt{computeFlip} is compared with
\texttt{minors(2,M)} in Macaulay2 and they are equal, and \(Z\) has
codimension three and degree four.

The output is then dissected:
...
Here the input is six equations rather than three, of degrees
\(2,2,2,3,3,3\):

\begin{verbatim}
y_5^2-y_3*y_6       y_4*y_5-y_2*y_6      y_3*y_4-y_2*y_5
y_2*y_4^2-y_1*y_6   y_2^2*y_4-y_1*y_5    y_2^3-y_1*y_3
\end{verbatim}

\noindent
All six survive in the output, joined by four that are linear in \(u\):

\begin{verbatim}
u_1*y_1-u_2*y_2       u_1*y_2^2-u_2*y_3
u_1*y_2*y_4-u_2*y_5   u_1*y_4^2-u_2*y_6
\end{verbatim}

\noindent
Nothing as tidy as the determinantal identity above holds here: the
\(2 \times 2\) minors of the corresponding \(2 \times 5\) matrix lie in
the ideal but do not generate it. What these equations do show is the
weighted fibre grading at work. The two generators of
\(I^{(2)} = (y_6, y_4^2)\) have degrees \(1\) and \(2\), so they are not
levelled; the fibre weights are \(\deg u_1 = (1,0)\) and
\(\deg u_2 = (1,1)\), and the last relation \(u_1 y_4^2 - u_2 y_6\) is
not bilinear. It is bihomogeneous only for those weights.
...
\item
  ...
  not for correctness.
\item
  For nontrivial fibre degrees \((p_j,e_j)\), the graph bridge chooses an
  integral slope \(D>\max(e_j/p_j)\).  The transformed weights
  \(Dp_j-e_j\) are positive, so the Hilbert-basis construction applies
  to the diagonal of bidegree \((n,Dn)\).  The weighted toric regression
  has degrees \((1,0),(1,1)\), hence \(D=2\) and weights \(2,1\).
\item
...
\item
  \textbf{Schwede, K., Smith, Z. et al.} The \texttt{WeilDivisors}
  package for Macaulay2, called \texttt{Divisor} through Macaulay2
  1.24. (Canonical divisors and divisorial ideals; used in place of
  \texttt{SymbolicPowers}.)
\end{itemize}
